...
c.JupyterHub.admin_access = True  # better switch to False in production
...
c.CILogonOAuthenticator.client_id = os.environ['OAUTH_CLIENT_ID']
c.CILogonOAuthenticator.client_secret = os.environ['OAUTH_CLIENT_SECRET']
...
with open(os.path.join(os.path.dirname(__file__), 'adminlist')) as f:
    for line in f:
        if not line:
            continue
        name = line.strip()
...
c.JupyterHub.cookie_secret_file = '/root/jupyterhub-conf/jupyterhub_cookie_secret'
...
c.JupyterHub.services = [
    {
        'name': 'cull-idle',
        'admin': True,
        'command': [sys.executable, '/srv/jupyterhub/cull_idle_servers.py', '--timeout=3600', '--max_age=86400'],
    }
]
...
c.JupyterHub.ssl_cert = '/root/jupyterhub-conf/star_renci_org.crt'

## Path to SSL key file for the public facing interface of the proxy
#
#  When setting this, you should also set ssl_cert
c.JupyterHub.ssl_key = '/root/jupyterhub-conf/star_renci_org.key'
